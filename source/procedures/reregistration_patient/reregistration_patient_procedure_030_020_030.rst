.. raw:: html

    <style> .red {color:red} </style>
    <style> .green {color:green} </style>
    <style> .blue {color:blue} </style>
    <style> .bi {font-weight: bold; font-style: italic} </style>

.. role:: red
.. role:: green
.. role:: blue
.. role:: bi

.. index:: Patient (Aux) Related Patient Create (Procedimento)

=========================================================
:bi:`Patient (Aux) Related Patient Create` (Procedimento)
=========================================================

    * *Workflow*: ":doc:`/reference_guide/reregistration_patient/reregistration_patient_workflow_030_020_010_030`".

    #. Acessar a *view* :bi:`Patients (Aux)`:

        * Menu de acesso:

            * :bi:`Health` » :bi:`Health` » :bi:`Patient` » :bi:`Patients (Aux)`

    #. Aplicar o filtro: **Agrupar Por** » :bi:`Phase` » :bi:`Register State` » :bi:`Verification State` » :bi:`Verification Outcome Informations`.

    #. Selecionar o(s) registro(s) desejado(s).

    #. Exercutar a Ação ":bi:`Patient (Aux) Related Patient Create`":

        #. Parâmetros apresentados:

            * *Patient (Aux) Set Code*: **marcado**
            * *Related Patient Verification Execute*: **marcado**
            * *Patient (Aux) Verification Execute*: **marcado**

        #. Utilizar o botão :bi:`Related Patient Create` para executar a Ação.

.. toctree::
   :maxdepth: 2
   :caption: Tópicos Relacionados:
