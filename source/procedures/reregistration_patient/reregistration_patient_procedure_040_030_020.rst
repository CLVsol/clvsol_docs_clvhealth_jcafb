.. raw:: html

    <style> .red {color:red} </style>
    <style> .green {color:green} </style>
    <style> .blue {color:blue} </style>
    <style> .bi {font-weight: bold; font-style: italic} </style>

.. role:: red
.. role:: green
.. role:: blue
.. role:: bi

.. index:: Patient Associate to Residence (Procedimento)

===================================================
:bi:`Patient Associate to Residence` (Procedimento)
===================================================

    * *Workflow*: ":doc:`/reference_guide/reregistration_patient/reregistration_patient_workflow_040_030_020`".

    #. Acessar a *view* :bi:`Patients`:

        * Menu de acesso:

            * :bi:`Health` » :bi:`Health` » :bi:`Patient` » :bi:`Patients`

    #. Aplicar o filtro: **Agrupar Por** » :bi:`Phase` » :bi:`Register State` » :bi:`Verification State` » :bi:`Verification Outcome Informations`.

    #. Selecionar o(s) registro(s) desejado(s).

    #. Executar a Ação ":bi:`Patient Associate to Residence`":

        #. Parâmetros apresentados:

            * *Create new Residence*: **marcado**

        #. Utilizar o botão :bi:`Associate to Residence` para executar a Ação.

    #. Executar a Ação ":bi:`Patient Verification Execute`":

        #. Parâmetros apresentados:

        #. Utilizar o botão :bi:`Patient Verification Execute` para executar a Ação.

.. toctree::
   :maxdepth: 2
   :caption: Tópicos Relacionados:
