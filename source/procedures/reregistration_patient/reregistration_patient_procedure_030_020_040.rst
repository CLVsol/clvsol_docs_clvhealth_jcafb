.. raw:: html

    <style> .red {color:red} </style>
    <style> .green {color:green} </style>
    <style> .blue {color:blue} </style>
    <style> .bi {font-weight: bold; font-style: italic} </style>

.. role:: red
.. role:: green
.. role:: blue
.. role:: bi

.. index:: Patient (Aux) Related Patient Update (Procedimento)

=========================================================
:bi:`Patient (Aux) Related Patient Update` (Procedimento)
=========================================================

    * *Workflow*: ":doc:`/reference_guide/reregistration_patient/reregistration_patient_workflow_030_020_020_010`".
    * *Workflow*: ":doc:`/reference_guide/reregistration_patient/reregistration_patient_workflow_030_020_020_020`".
    * *Workflow*: ":doc:`/reference_guide/reregistration_patient/reregistration_patient_workflow_030_020_020_030`".

    #. Acessar a *view* :bi:`Patients (Aux)`:

        * Menu de acesso:

            * :bi:`Health` » :bi:`Health` » :bi:`Patient` » :bi:`Patients (Aux)`

    #. Aplicar o filtro: **Agrupar Por** » :bi:`Phase` » :bi:`Register State` » :bi:`Verification State` » :bi:`Verification Outcome Informations`.

    #. Selecionar o(s) registro(s) desejado(s).

    #. Executar a Ação ":bi:`Patient (Aux) Related Patient Update`":

        #. Parâmetros apresentados:

            * *Update Contact Information Data*: **marcado**
            * *Related Patient Verification Execute*: **marcado**
            * *Patient (Aux) Verification Execute*: **marcado**

        #. Utilizar o botão :bi:`Related Patient Update` para executar a Ação.

.. toctree::
   :maxdepth: 2
   :caption: Tópicos Relacionados:
