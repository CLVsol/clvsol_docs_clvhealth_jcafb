...
============================================
    ...
    No caso de uma Pessoa "**já cadastrada**", os registros relativos a ela serão automaticamente criados no **Cadastro Auxiliar** (:bi:`Person (Aux)` e :bi:`Address (Aux)`) a partir das informações dos registros equivalentes presentes no **Cadastro** (:bi:`Person`, :bi:`Address` e :bi:`Familiy`).
    ...
    No caso de uma Pessoa "**não cadastrada**", os registros no **Cadastro Auxiliar** relativos a ela (:bi:`Person (Aux)` e :bi:`Addresse (Aux)`) será criado com as informações apresentadas para a Pessoa.
