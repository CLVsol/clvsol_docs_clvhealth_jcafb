...
=============================
    ...
    O Processo de Consolidação de :bi:`Patient` é feito guiando-se pelo :bi:`Verification State` e :bi:`Verification Outcomes` de cada registro :bi:`Patient`.
    ...
    Os estados diferentes de "**Ok**" são classificados em 3 níveis de "**L0**", "**L1**" e "**L2**" (não existe no momento a ocorrencia do nivel "**L2**"):
      ...
      * "**L0**": ("Error (L0)" ou "Warning (L0)") sinalizam um Erro ou Advertência ocorrendo no próprio registro :bi:`Patient`.

      * "**L1**": ("Error (L1)" ou "Warning (L1)") sinalizam um Erro ou Advertência ocorrendo no registro de uma outra Entidade de Cadastro relacionada ao registro :bi:`Patient`.
    ...
    De forma geral, o Processo de Consolidação de :bi:`Patient` segue a ordem:

      #. :doc:`reregistration_workflow_040_030_010`

      #. :doc:`reregistration_workflow_040_030_020`
